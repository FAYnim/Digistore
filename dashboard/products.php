...
<div class="modal fixed inset-0 z-50 place-items-center bg-slate-950/60 p-4" id="productModal"><div class="card max-h-[90vh] w-full max-w-3xl overflow-auto p-5"><div class="mb-4 flex items-center justify-between"><h3 class="text-xl font-black" id="productModalTitle">Tambah Produk</h3><button class="btn-soft" data-close-modal type="button">Tutup</button></div><form class="grid gap-3 md:grid-cols-2" id="productForm"><input type="hidden" id="productId"><label>Nama Produk<input class="input mt-1" id="productName" required placeholder="Google AI Pro"></label><label>Harga<input class="input mt-1" id="productPrice" required type="number" placeholder="25000"></label><label>Harga Coret<input class="input mt-1" id="productOriginalPrice" type="number" placeholder="50000"></label><label>Status<select class="select mt-1" id="productStatus"><option value="active">Aktif</option><option value="draft">Draft</option><option value="out_of_stock">Habis</option></select></label><label class="md:col-span-2">Data Akun Premium<textarea class="textarea mt-1" id="productAccounts" name="accounts_text" rows="8" placeholder="1 akun per baris, contoh:&#10;email@example.com | password123 | PIN 1234"></textarea><small class="text-xs font-bold text-slate-500 dark:text-slate-400">Masukkan akun baru saja. Akun yang sudah tersimpan tidak perlu diinput ulang.</small><div id="productAccountsSummary" class="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400"></div></label><label>Gambar URL<input class="input mt-1" id="productImage" placeholder="https://placehold.co/600x400"></label><label class="md:col-span-2">Deskripsi<textarea class="textarea mt-1" id="productDescription" rows="3" placeholder="Lorem ipsum dolor sit amet."></textarea></label><label class="flex items-center gap-2 font-bold"><input id="productFeatured" type="checkbox"> Featured</label><div class="flex justify-end gap-2 md:col-span-2"><button class="btn-soft" data-close-modal type="button">Batal</button><button class="btn-primary" type="submit">Simpan</button></div></form></div></div>
