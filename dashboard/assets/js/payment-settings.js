...
function renderPaymentPreview() {
  ...
  const whatsapp = paymentValue('#paymentAdminWhatsapp') || paymentDefaults.payment_admin_whatsapp;
  const message = paymentValue('#paymentWhatsappMessage') || paymentDefaults.payment_whatsapp_message;
  const waText = encodeURIComponent(message.replace('{order_code}', 'ORD-20260624-A8K3').replace('{customer_name}', 'Budi').replace('{total_amount}', 'Rp25.000'));
  ...
  document.querySelector('#paymentPreview').innerHTML = `
    ...
    <a class="btn-primary mt-5 inline-flex" href="https://wa.me/${paymentEscape(whatsapp)}?text=${waText}" target="_blank" rel="noopener">Konfirmasi WhatsApp</a>
  `;
}
